...
impl Mempool {
	pub fn spawn(
		...
		let mempool = Self { name, committee, parameters, store, tx_consensus };
	...
	fn handle_consensus_messages(&self, rx_consensus: Receiver<ConsensusMempoolMessage>) {
		// The `Synchronizer` is responsible to keep the mempool in sync with the others. It handles
		// the commands it receives from the consensus (which are mainly notifications that we are
		// out of sync).
		Synchronizer::spawn(
			self.name,
	...
	fn handle_clients_transactions(&self) {
		...
		// broadcasts (in a reliable manner) the batches to all other mempools that share the same
		// `id` as us. Finally, it gathers the 'cancel handlers' of the messages and send them to
		// the `QuorumWaiter`.
		BatchMaker::spawn(
		...
		// The `Processor` hashes and stores the batch. It then forwards the batch's digest to the
		// consensus.
		Processor::spawn(
			self.store.clone(),
	...
	fn handle_mempool_messages(&self) {
		...
		NetworkReceiver::spawn(
			address,
			/* handler */
			MempoolReceiverHandler { tx_helper, tx_processor },
		);

		// The `Helper` is dedicated to reply to batch requests from other mempools.
		Helper::spawn(self.committee.clone(), self.store.clone(), /* rx_request */ rx_helper);

		// This `Processor` hashes and stores the batches we receive from the other mempools. It
		// then forwards the batch's digest to the consensus.
		Processor::spawn(
			self.store.clone(),
			/* rx_batch */ rx_processor,
			/* tx_digest */ self.tx_consensus.clone(),
	...
	async fn dispatch(&self, writer: &mut Writer, serialized: Bytes) -> Result<(), Box<dyn Error>> {
		...
		match bincode::deserialize(&serialized) {
			Ok(MempoolMessage::Batch(..)) =>
				self.tx_processor.send(serialized.to_vec()).await.expect("Failed to send batch"),
