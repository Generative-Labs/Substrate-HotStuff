...
impl Block {
	pub fn new_from_key(
		...
		let block = Block { qc, tc: None, author, round, payload, signature: Signature::default() };
...
impl Vote {
	pub fn new_from_key(hash: Digest, round: Round, author: PublicKey, secret: &SecretKey) -> Self {
		let vote = Self { hash, round, author, signature: Signature::default() };
...
// Fixture.
pub fn qc() -> QC {
	let qc = QC { hash: Digest::default(), round: 1, votes: Vec::new() };
...
pub fn chain(keys: Vec<(PublicKey, SecretKey)>) -> Vec<Block> {
	...
	keys.iter()
		...
		.map(|(i, key)| {
			...
			let qc = QC { hash: block.digest(), round: block.round, votes: Vec::new() };
			let digest = qc.digest();
