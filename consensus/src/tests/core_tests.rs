...
fn core(
	...
	committee: Committee,
	store_path: &str,
) -> (Sender<ConsensusMessage>, Receiver<ProposerMessage>, Receiver<Block>) {
...
fn leader_keys(round: Round) -> (PublicKey, SecretKey) {
	...
	keys().into_iter().find(|(public_key, _)| *public_key == leader).unwrap()
}
...
async fn generate_proposal() {
	...
	let hight_qc = QC {
		hash,
		round: block.round,
		votes: votes.iter().cloned().map(|x| (x.author, x.signature)).collect(),
	};
