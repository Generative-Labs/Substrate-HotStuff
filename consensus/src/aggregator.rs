...
impl Aggregator {
	pub fn new(committee: Committee) -> Self {
		Self { committee, votes_aggregators: HashMap::new(), timeouts_aggregators: HashMap::new() }
	}
...
impl QCMaker {
	pub fn new() -> Self {
		Self { weight: 0, votes: Vec::new(), used: HashSet::new() }
	}
	...
	pub fn append(&mut self, vote: Vote, committee: &Committee) -> ConsensusResult<Option<QC>> {
		...
		ensure!(self.used.insert(author), ConsensusError::AuthorityReuse(author));
...
impl TCMaker {
	...
	pub fn append(
		...
		self.votes.push((author, timeout.signature, timeout.high_qc.round));
		self.weight += committee.stake(&author);
		if self.weight >= committee.quorum_threshold() {
			self.weight = 0; // Ensures TC is only created once.
			return Ok(Some(TC { round: timeout.round, votes: self.votes.clone() }))
		}
		Ok(None)
	}
}
