...
impl Block {
	pub async fn new(
		...
		let block = Self { qc, tc, author, round, payload, signature: Signature::default() };
...
impl Vote {
	...
	pub fn verify(&self, committee: &Committee) -> ConsensusResult<()> {
		// Ensure the authority has voting rights.
		ensure!(committee.stake(&self.author) > 0, ConsensusError::UnknownAuthority(self.author));
...
impl QC {
	...
	pub fn verify(&self, committee: &Committee) -> ConsensusResult<()> {
		...
		ensure!(weight >= committee.quorum_threshold(), ConsensusError::QCRequiresQuorum);
...
impl Timeout {
	pub async fn new(
		...
		let timeout = Self { high_qc, round, author, signature: Signature::default() };
...
impl TC {
	pub fn verify(&self, committee: &Committee) -> ConsensusResult<()> {
		...
		ensure!(weight >= committee.quorum_threshold(), ConsensusError::TCRequiresQuorum);
