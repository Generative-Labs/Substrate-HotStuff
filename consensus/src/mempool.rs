...
impl MempoolDriver {
	pub fn new(
		...
		Self { store, tx_mempool, tx_payload_waiter }
	}
	...
	pub async fn verify(&mut self, block: Block) -> ConsensusResult<bool> {
		...
		self.tx_mempool.send(message).await.expect("Failed to send sync message");
...
impl PayloadWaiter {
	pub fn spawn(
		...
		tokio::spawn(async move {
			Self { store, rx_message, tx_loopback }.run().await;
		});
	}
