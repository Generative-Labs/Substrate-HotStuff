...
impl Consensus {
	...
	pub fn spawn(
		...
		let mut address = committee.address(&name).expect("Our public key is not in the committee");
		...
		info!("Node {} listening to consensus messages on {}", name, address);
	...
	async fn dispatch(&self, writer: &mut Writer, serialized: Bytes) -> Result<(), Box<dyn Error>> {
		...
		match bincode::deserialize(&serialized).map_err(ConsensusError::SerializationError)? {
			...
			message @ ConsensusMessage::Propose(..) => {
				...
				self.tx_consensus.send(message).await.expect("Failed to consensus message")
			},
			message => self.tx_consensus.send(message).await.expect("Failed to consensus message"),
		}
		Ok(())
	}
}
