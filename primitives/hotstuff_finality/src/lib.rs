...
//! Primitives for HOTSTUFF integration, suitable for WASM compilation.
...
pub const GRANDPA_AUTHORITIES_KEY: &[u8] = b":hotstuff_authorities";
...
/// A hotstuff message for a substrate chain.
pub type Message<Header> = finality_hotstuff::Message<<Header as HeaderT>::Hash, <Header as HeaderT>::Number>;

/// A signed message.
pub type SignedMessage<Header> = finality_hotstuff::SignedMessage<
...
pub type PrimaryPropose<Header> =
	finality_hotstuff::PrimaryPropose<<Header as HeaderT>::Hash, <Header as HeaderT>::Number>;
/// A prevote message for this chain's block type.
pub type Prevote<Header> = finality_hotstuff::Prevote<<Header as HeaderT>::Hash, <Header as HeaderT>::Number>;
/// A precommit message for this chain's block type.
pub type Precommit<Header> =
	finality_hotstuff::Precommit<<Header as HeaderT>::Hash, <Header as HeaderT>::Number>;
/// A catch up message for this chain's block type.
pub type CatchUp<Header> = finality_hotstuff::CatchUp<
	<Header as HeaderT>::Hash,
	<Header as HeaderT>::Number,
	AuthoritySignature,
	AuthorityId,
...
pub type Commit<Header> = finality_hotstuff::Commit<
	<Header as HeaderT>::Hash,
	<Header as HeaderT>::Number,
	AuthoritySignature,
	AuthorityId,
...
pub type CompactCommit<Header> = finality_hotstuff::CompactCommit<
	<Header as HeaderT>::Hash,
	<Header as HeaderT>::Number,
	AuthoritySignature,
	AuthorityId,
...
/// A HOTSTUFF justification for block finality, it includes a commit message and
...
pub struct HotstuffJustification<Header: HeaderT> {
	pub round: u64,
...
pub enum Equivocation<H, N> {
	...
	Prevote(finality_hotstuff::Equivocation<AuthorityId, finality_hotstuff::Prevote<H, N>, AuthoritySignature>),
	/// Proof of equivocation at precommit stage.
	Precommit(finality_hotstuff::Equivocation<AuthorityId, finality_hotstuff::Precommit<H, N>, AuthoritySignature>),
}

impl<H, N> From<finality_hotstuff::Equivocation<AuthorityId, finality_hotstuff::Prevote<H, N>, AuthoritySignature>>
	for Equivocation<H, N>
{
	fn from(
		equivocation: finality_hotstuff::Equivocation<
			AuthorityId,
			finality_hotstuff::Prevote<H, N>,
			AuthoritySignature,
		>,
	) -> Self {
...
impl<H, N> From<finality_hotstuff::Equivocation<AuthorityId, finality_hotstuff::Precommit<H, N>, AuthoritySignature>>
	for Equivocation<H, N>
{
	fn from(
		equivocation: finality_hotstuff::Equivocation<
			AuthorityId,
			finality_hotstuff::Precommit<H, N>,
			AuthoritySignature,
		>,
	) -> Self {
	...
	N: Clone + Encode + PartialEq,
{
	...
	match report.equivocation {
		Equivocation::Prevote(equivocation) => {
			check!(equivocation, finality_hotstuff::Message::Prevote);
		},
		Equivocation::Precommit(equivocation) => {
			check!(equivocation, finality_hotstuff::Message::Precommit);
		},
	}
}
...
pub fn check_message_signature<H, N>(
	message: &finality_hotstuff::Message<H, N>,
	id: &AuthorityId,
...
pub fn sign_message<H, N>(
	...
	round: RoundNumber,
	set_id: SetId,
) -> Option<finality_hotstuff::SignedMessage<H, N, AuthoritySignature, AuthorityId>>
where
	H: Encode,
	N: Encode,
{
	...
	Some(finality_hotstuff::SignedMessage { message, signature, id: public })
}
...
sp_api::decl_runtime_apis! {
	...
	pub trait HotstuffFinalityApi {
		...
		fn hotstuff_authorities() -> AuthorityList;
