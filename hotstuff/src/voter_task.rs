...
use sc_client_api::{Backend, BlockImportNotification, CallExecutor};
...
use sp_consensus_hotstuff::{
	sr25519::AuthorityId as HotstuffId, Slot, HOTSTUFF_ENGINE_ID, HOTSTUFF_KEY_TYPE,
};
use sp_core::{traits::CallContext, ByteArray, Decode, Encode};
	...
	S: SyncingT<Block> + Sync + 'static,
{
	pub fn new(
		...
		Self {
			client,
			network,
			voted_block_set: HashSet::default(),
			key_store,
	...
	pub fn do_finalize_block(&self, hash: Block::Hash) {
	...
	fn is_block_author(&self, block_header: &Block::Header) -> Result<bool, HotstuffError> {
		...
		for item in block_header.digest().logs() {
			if let Some(mut cons) = item.as_pre_runtime() {
				...
				let slot =
					Slot::decode(&mut cons.1).map_err(|e| HotstuffError::Other(e.to_string()))?;

				let author_index = *slot % authorities.len() as u64;

				if self.key_store.has_keys(&[(
					authorities[author_index as usize].to_raw_vec(),
					HOTSTUFF_KEY_TYPE,
				)]) {
					return Ok(true)
				}
	...
	fn income_message_handler(&mut self, notification: TopicNotification) {
		// let message: gossip::GossipMessage::<Block> = Decode::decode(&mut
		// &notification.message[..]).unwrap();
		...
		let decoded_message =
			gossip::GossipMessage::<Block>::decode(&mut &notification.message[..]);
				...
				match sender_peer_id {
					Ok(peer_id) => {
						...
						self.network
							.gossip_engine
							.lock()
							.send_message(vec![peer_id], vote_message.encode());
					},
					Err(err) => {
						eprintln!("Error: {}", err);
					},
	...
	fn block_import_handler(&self, notification: BlockImportNotification<Block>) {
		...
		info!(
			">>> Simple voter get block from block_import, header_number {},header_hash:{}",
			header.number(),
			header.hash()
		);
		...
		// If the gossip engine detects that a message received from the network has already been
		// registered or is pending broadcast, it will not be reported to the upper-level receivers.
		...
		//     id,
		// };
		let topic: <Block as BlockT>::Hash =
			<<Block::Header as HeaderT>::Hashing as HashT>::hash(b"hotstuff/vote");

		let consensus_message =
			gossip::GossipMessage::Consensus(gossip::ConsensusMessage::<Block> {
				topic,
				round: 1,
				peer_id: self.network.local_peer_id().to_bytes(),
			...
			Ok(is_author) =>
				if is_author {
					...
					self.network
						.gossip_engine
						.lock()
						.register_gossip_message(topic, consensus_message.encode());
				} else {
	...
	S: SyncingT<Block> + Sync + 'static,
{
	...
	fn poll(mut self: Pin<&mut Self>, cx: &mut Context) -> Poll<Self::Output> {
		let mut notification: TracingUnboundedReceiver<BlockImportNotification<Block>> =
