...
use rand::{
	rngs::{OsRng, StdRng},
	CryptoRng, RngCore, SeedableRng as _,
};
...
impl PublicKey {
	...
	pub fn decode_base64(s: &str) -> Result<Self, base64::DecodeError> {
		...
		let array = bytes[..32].try_into().map_err(|_| base64::DecodeError::InvalidLength)?;
		Ok(Self(array))
	}
}
...
impl SecretKey {
	...
	pub fn decode_base64(s: &str) -> Result<Self, base64::DecodeError> {
		let bytes = base64::decode(s)?;
		let array = bytes[..64].try_into().map_err(|_| base64::DecodeError::InvalidLength)?;
		Ok(Self(array))
	}
}
