...
impl<T: Config> Pallet<T> {
	...
	pub fn change_authorities(new: BoundedVec<T::AuthorityId, T::MaxAuthorities>) {
		...
		log::info!(">>> change_authorities aura");
	...
	pub fn initialize_authorities(authorities: &[T::AuthorityId]) {
		log::info!(">>> initialize_authorities aura");
	...
	pub fn do_try_state() -> Result<(), sp_runtime::TryRuntimeError> {
		...
		log::info!(">>> do_try_state aura {}", current_slot);
...
impl<T: Config> OneSessionHandler<T::AccountId> for Pallet<T> {
	...
	where
		I: Iterator<Item = (&'a T::AccountId, T::AuthorityId)>,
	{
		log::info!(">>> on_new_session aura changed: {}", changed);
