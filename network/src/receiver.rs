...
pub trait MessageHandler: Clone + Send + Sync + 'static {
	/// Defines how to handle an incoming message. A typical usage is to define a `MessageHandler`
	/// with a number of `Sender<T>` channels. Then implement `dispatch` to deserialize incoming
	/// messages and forward them through the appropriate delivery channel. Then `writer` can be
	/// used to send back responses or acknowledgements to the sender machine (see unit tests for
	/// examples).
	async fn dispatch(&self, writer: &mut Writer, message: Bytes) -> Result<(), Box<dyn Error>>;
}

/// For each incoming request, we spawn a new runner responsible to receive messages and forward
/// them through the provided deliver channel.
...
impl<Handler: MessageHandler> Receiver<Handler> {
	...
	async fn run(&self) {
		let listener = TcpListener::bind(&self.address).await.expect("Failed to bind TCP port");
	...
	/// Spawn a new runner to handle a specific TCP connection. It receives messages and process
	/// them using the provided handler.
