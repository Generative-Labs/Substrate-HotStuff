...
/// A connection is responsible to establish and keep alive (if possible) a connection with a single
/// peer.
struct Connection {
...
impl Connection {
	...
	async fn run(&mut self) {
		...
		let (mut writer, mut reader) = match TcpStream::connect(self.address).await {
			...
			Err(e) => {
				warn!("{}", NetworkError::FailedToConnect(self.address, /* retry */ 0, e));
				return
			},
		};
