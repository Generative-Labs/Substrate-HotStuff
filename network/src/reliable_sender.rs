...
/// We keep alive one TCP connection per peer, each connection is handled by a separate task (called
/// `Connection`). We communicate with our 'connections' through a dedicated channel kept by the
/// HashMap called `connections`. This sender is 'reliable' in the sense that it keeps trying to
/// re-transmit messages for which it didn't receive an ACK back (until they succeed or are
/// canceled).
...
impl ReliableSender {
	pub fn new() -> Self {
		Self { connections: HashMap::new(), rng: SmallRng::from_entropy() }
	}
	...
	pub async fn send(&mut self, address: SocketAddr, data: Bytes) -> CancelHandler {
		...
		self.connections
			...
			.send(InnerMessage { data, cancel_handler: sender })
			.await
			.expect("Failed to send internal message");
		receiver
	}

	/// Broadcast the message to all specified addresses in a reliable manner. It returns a vector
	/// of cancel handlers ordered as the input `addresses` vector.
...
/// A connection is responsible to reliably establish (and keep alive) a connection with a single
/// peer.
struct Connection {
...
impl Connection {
	fn spawn(address: SocketAddr, receiver: Receiver<InnerMessage>) {
		tokio::spawn(async move {
			Self { address, receiver, retry_delay: 200, buffer: VecDeque::new() }
				.run()
				.await;
		});
	}
		...
		loop {
			match TcpStream::connect(self.address).await {
				Ok(stream) => {
					...
					// messages. The following function only returns if there is an error.
	...
	async fn keep_alive(&mut self, stream: TcpStream) -> NetworkError {
		...
		let error = 'connection: loop {
			...
			// Check if there are any new messages to send or if we get an ACK for messages we
			// already sent.
			tokio::select! {
				Some(InnerMessage{data, cancel_handler}) = self.receiver.recv() => {
		...
		// If we reach this code, it means something went wrong. Put the messages for which we
		// didn't receive an ACK back into the sending buffer, we will try to send them again once
		// we manage to establish a new connection.
