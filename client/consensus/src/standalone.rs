...
pub async fn claim_slot<P: Pair>(
	...
	expected_author.and_then(|p| {
		if keystore.has_keys(&[(p.to_raw_vec(), sp_consensus_hotstuff::HOTSTUFF_KEY_TYPE)]) {
			Some(p.clone())
		} else {
			None
		}
...
where
	P::Signature: Codec,
{
	<DigestItem as CompatibleDigestItem<P::Signature>>::hotstuff_pre_digest(slot)
}
	...
	P::Public: AppPublic,
{
	...
	let signature_digest_item =
		<DigestItem as CompatibleDigestItem<P::Signature>>::hotstuff_seal(signature);
	...
	P::Public: Codec + PartialEq + Clone,
{
	...
	let sig = seal.as_hotstuff_seal().ok_or(SealVerificationError::BadSeal)?;
