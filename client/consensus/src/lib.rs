...
//!
//! NOTE: Hotstuff itself is designed to be generic over the crypto used.
...
pub struct StartHotstuffParams<C, SC, I, PF, SO, L, CIDP, BS, N> {
...
/// Start the hotstuff worker. The returned future should be run in a futures executor.
pub fn start_hotstuff<P, B, C, SC, I, PF, SO, L, CIDP, BS, Error>(
	StartHotstuffParams {
		slot_duration,
		client,
		select_chain,
		...
		telemetry,
		compatibility_mode,
	}: StartHotstuffParams<C, SC, I, PF, SO, L, CIDP, BS, NumberFor<B>>,
...
{
	let worker = build_hotstuff_worker::<P, _, _, _, _, _, _, _, _>(BuildHotstuffWorkerParams {
		client,
		block_import,
		proposer_factory,
		keystore,
...
/// Parameters of [`build_hotstuff_worker`].
pub struct BuildHotstuffWorkerParams<C, I, PF, SO, L, BS, N> {
	/// The client to interact with the chain.
	pub client: Arc<C>,
	/// The block import.
	pub block_import: I,
...
/// Build the Hotstuff worker.
///
/// The caller is responsible for running this worker, otherwise it will do nothing.
pub fn build_hotstuff_worker<P, B, C, PF, I, SO, L, BS, Error>(
	BuildHotstuffWorkerParams {
		client,
		block_import,
		proposer_factory,
		sync_oracle,
		...
		force_authoring,
		compatibility_mode,
	}: BuildHotstuffWorkerParams<C, I, PF, SO, L, BS, NumberFor<B>>,
...
struct HotstuffWorker<C, E, I, P, SO, L, BS, N> {
	client: Arc<C>,
	block_import: I,
	env: E,
...
impl<B, C, E, I, P, Error, SO, L, BS> sc_consensus_slots::SimpleSlotWorker<B>
	for HotstuffWorker<C, E, I, P, SO, L, BS, NumberFor<B>>
where
	B: BlockT,
...
// 	use sp_consensus_hotstuff::HOTSTUFF_KEY_TYPE;
// 	use sp_consensus::{DisableProofRecording, NoNetwork as DummyOracle, Proposal};
// 	use sp_consensus_hotstuff::sr25519::AuthorityPair;
...
// 	type HotstuffVerifier = import_queue::HotstuffVerifier<
// 		PeersFullClient,
// 		AuthorityPair,
// 		Box<
...
// 		u64,
// 	>;
// 	type HotstuffPeer = Peer<(), PeersClient>;

// 	#[derive(Default)]
// 	pub struct HotstuffTestNet {
// 		peers: Vec<HotstuffPeer>,
// 	}

// 	impl TestNetFactory for HotstuffTestNet {
// 		type Verifier = HotstuffVerifier;
// 		type PeerData = ();
...
// 			import_queue::HotstuffVerifier::new(
// 				client,
// 				Box::new(|_, _| async {
...
// 			keystore
// 				.sr25519_generate_new(HOTSTUFF_KEY_TYPE, Some(&key.to_seed()))
...
// 			hotstuff_futures.push(
// 				start_hotstuff::<AuthorityPair, _, _, _, _, _, _, _, _, _, _>(StartHotstuffParams {
// 					slot_duration,
...
// 				Poll::<()>::Pending
// 			}),
// 			future::select(future::join_all(hotstuff_futures), future::join_all(import_notifications)),
// 		)
// 		.await;
// 	}
