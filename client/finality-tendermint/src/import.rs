...
/// A block-import handler for TDMT.
///
...
///
/// When using TDMT, the block import worker should be using this block import
/// object.
...
pub fn find_scheduled_change<B: BlockT>(
	...
	let id = OpaqueDigestItemId::Consensus(&TDMT_ENGINE_ID);
	...
	TransactionFor<Client, Block>: 'static,
{
	...
	fn current_set_id(&self, id: &BlockId<Block>) -> Result<SetId, ConsensusError> {
		...
		if runtime_version.api_version(&<dyn TendermintApi<Block>>::ID).map_or(false, |v| v < 3) {
		...
		match import_result {
			Ok(ImportResult::Imported(aux)) => {
				...
				let authorities = self
					.inner
					.runtime_api()
					.tendermint_authorities(&block_id)
					.map_err(|e| ConsensusError::ClientImport(e.to_string()))?;
...
#[async_trait::async_trait]
impl<BE, Block: BlockT, Client, SC> BlockImport<Block> for TendermintBlockImport<BE, Block, Client, SC>
where
	NumberFor<Block>: finality_tendermint::BlockNumberOps,
	BE: Backend<Block>,
	Client: ClientForTendermint<Block, BE>,
	...
	SC: Send,
{
	...
	async fn import_block(
		...
		let tendermint_justification =
			justifications.and_then(|just| just.into_justification(TDMT_ENGINE_ID));
	...
	NumberFor<Block>: finality_tendermint::BlockNumberOps,
{
	...
	fn import_justification(
		...
		if justification.0 != TDMT_ENGINE_ID {
