...
use finality_tendermint::{messages, voter};
use futures::{
	prelude::*,
...

		{
			...
			match input {
				messages::GlobalMessageIn::Commit(_, ref commit, ..) => {
					...
					let commit_targets =
						commit.commits.iter().map(|c| (c.target_height, c.target_hash));

					for (target_number, target_hash) in commit_targets {
                        let target_hash = target_hash.expect("commit messages always have a target hash; qed");
